...
export default function BlogPostPage({
	...
	const headingItems = useMemo(
		() => extractPortableTextHeadingItems(post.body),
		[post.body],
	);
		...
		const loadArticleEngagement = async () => {
			try {
				...
				if (likeResult.success) {
					setLikeCount(Number(likeResult.data?.likeCount ?? 0));
					setUserLiked(Boolean(likeResult.data?.userLiked));
				}
				if (bookmarkResult.success) {
					setUserBookmarked(Boolean(bookmarkResult.data?.userBookmarked));
				}
			} catch {
	...
	const onToggleArticleLike = async () => {
		...
		try {
			...
			if (!result.success) {
				if (result.code === "ARTICLE_LIKE_LOGIN_REQUIRED") {
					showAuthRequiredDialog(t("articleLikeLoginRequired"));
					return;
				}
				setLikeError(t("articleLikeError"));
			}
	...
	const onToggleArticleBookmark = async () => {
		...
		try {
			const result = userBookmarked
				? await unbookmarkArticle(articleKey)
				: await bookmarkArticle(articleKey);
			if (!result.success) {
				if (result.code === "ARTICLE_BOOKMARK_LOGIN_REQUIRED") {
					showAuthRequiredDialog(t("articleBookmarkLoginRequired"));
					return;
				}
				setBookmarkError(t("articleBookmarkError"));
			}
			...
			<div className="min-h-screen bg-background">
				...
				<div className="mx-auto flex max-w-300 justify-center gap-3 px-5 sm:px-8 lg:gap-4">
					<article className="min-w-0 w-full max-w-3xl lg:flex-1">
						{/* Back button */}
						<div
							className={coverSrc ? "-mt-8 sm:-mt-10 relative z-10" : "pt-16"}
						>
							<Button
								variant="ghost"
								size="sm"
						...
						<div className="sticky top-24 space-y-4">
							<div className="flex w-fit flex-col items-center gap-1 rounded-2xl border border-border py-3 px-3">
								...
								<Link
									...
									aria-label={t("commentsCountShort", {
